...
const TransferCertificate = () => {
    ...
    const handlePrintTc = (tc) => {
        const dateStr = new Date(tc.issue_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' });
        const studentName = tc.student_name ? `${tc.student_name}` : `STU-${String(tc.student_id).padStart(3, '0')}`;
        
        const content = `
================================================================================
                            TRANSFER CERTIFICATE
================================================================================
TC Number:      ${tc.tc_number}
Issue Date:     ${dateStr}
Status:         ${tc.status.toUpperCase()}

This is to certify that student ${studentName} has been officially
transferred from the institution.

Reason for Transfer:
${tc.reason}

Issued by: ${tc.issued_by || 'School Administration'}

--------------------------------------------------------------------------------
         This is a digitally verified academic record. No physical signature is required.
================================================================================
        `;
        ...
        link.setAttribute("download", `TC_${tc.tc_number}.txt`);
    ...
    const handleAddSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await tcApi.createTransferCertificate(newTc);
            if (res) {
                toast.success("TC record created successfully!");
            ...
            const mockTc = {
                tc_id: Math.max(...allTcs.map(t => t.tc_id), 0) + 1,
                ...newTc,
                issue_date: new Date(newTc.issue_date).toISOString()
            };
            setAllTcs(prev => [mockTc, ...prev]);
            toast.success("TC record created (Local Mockup Mode)");
            setIsAddModalOpen(false);
        }
    };
        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button icon={Plus} onClick={() => setIsAddModalOpen(true)}>Generate TC</Button>
                </div>
            </div>
                    ...
                    renderActions={(r) => (
                        <div className="flex justify-end gap-1">
                            {r.status === 'pending' && (
                             ...
                             <button onClick={() => handlePrintTc(r)} className="p-2 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 rounded-lg transition-all" title="Print/Download TC">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center justify-between pt-2">
                            ...
                            <div className="flex gap-2">
                                <Button variant="secondary" onClick={() => handlePrintTc(viewingTc)} icon={Printer}>Print</Button>
                ...
                <form onSubmit={handleAddSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input 
                                type="number" 
                                value={newTc.student_id} 
                                onChange={(e) => setNewTc({...newTc, student_id: e.target.value})}
                                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-primary rounded-xl text-sm font-semibold text-slate-700 outline-none"
                                required
                                placeholder="e.g. 1"
                            />
                        ...
                        <div>
                            ...
                            <input 
                                type="text" 
                                value={newTc.issued_by} 
                                onChange={(e) => setNewTc({...newTc, issued_by: e.target.value})}
                                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-primary rounded-xl text-sm font-semibold text-slate-700 outline-none"
                    ...
                    <div>
                        ...
                        <textarea 
                            value={newTc.reason} 
                            onChange={(e) => setNewTc({...newTc, reason: e.target.value})}
                            className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-primary rounded-xl text-sm font-semibold text-slate-700 outline-none h-20"
                            required
                    ...
                    <div>
                        ...
                        <select 
                            value={newTc.status} 
                            onChange={(e) => setNewTc({...newTc, status: e.target.value})}
                            className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-primary rounded-xl text-sm font-semibold text-slate-700 outline-none h-11"
                            required
                        >
                            <option value="pending">pending</option>
