...
const Certificates = () => {
    ...
    const handlePrintCert = (cert) => {
        const certName = cert.certificate_type || 'Bonafide Certificate';
        const dateStr = new Date(cert.issued_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' });
        const studentName = cert.student_name ? `${cert.student_name}` : `STU-${String(cert.student_id).padStart(3, '0')}`;
...
This is to certify that student ${studentName} has been issued a
"${certName}" by the institution.

This credential is officially recorded in the school ERP registry and verified.

Issued by: ${cert.issued_by || 'School Administration'}
...
================================================================================
        ...
        link.setAttribute("download", `Certificate_${cert.certificate_no}.txt`);
    ...
    const handleAddSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await certificateApi.createCertificate(newCert);
            if (res) {
            ...
            const mockCert = {
                certification_id: Math.max(...allCertificates.map(c => c.certification_id), 0) + 1,
                ...newCert,
                issued_date: new Date(newCert.issued_date).toISOString()
            };
            setAllCertificates(prev => [mockCert, ...prev]);
            toast.success("Certificate issued (Local Mockup Mode)");
            setIsAddModalOpen(false);
        }
    };
        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button icon={Plus} onClick={() => setIsAddModalOpen(true)}>Issue Certificate</Button>
                </div>
            </div>
                ...
                <Table
                    ...
                    renderActions={(r) => (
                        <div className="flex justify-end gap-1">
                            ...
                            <button onClick={() => handlePrintCert(r)} className="p-2 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 rounded-lg transition-all" title="Print Certificate">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center justify-between pt-2">
                            ...
                            <div className="flex gap-2">
                                <Button variant="secondary" onClick={() => handlePrintCert(viewingCert)} icon={Printer}>Print</Button>
