...
const Reports = () => {
  ...
  const fetchReportData = useCallback(async () => {
    ...
    try {
      ...
      const logsData = await apiClient.get('/merge-log'); // Using merge logs as placeholder for activity/report logs
  ...
  const handleDownloadReport = (row) => {
    const reportName = row.action || row.merge_reason || 'System Audit';
    const dateStr = new Date(row.created_at || row.merge_date || new Date()).toLocaleDateString();
    const content = `
================================================================================
                               SYSTEM REPORT
================================================================================
Report Title:   ${reportName}
Category:       ${(row.status || '').toLowerCase() === 'success' || (row.status || '').toLowerCase() === 'completed' ? 'Management Report' : 'System Report'}
Generated On:   ${dateStr}
Status:         ${row.status || 'Success'}

Details:
${row.details || row.merge_reason || 'This audit report compiles all records and operations logged during this cycle.'}

--------------------------------------------------------------------------------
         This document is officially generated by the School ERP System.
================================================================================
    `;
    ...
    addToast(`${reportName} downloaded successfully!`, 'success');
  };
  ...
  const handleGenerateReport = async () => {
    try {
      const newReport = {
        ...
        status: 'Success',
        details: 'Verified database connection pools, memory utilization, and active session counts.'
      };
      
      const res = await apiClient.post('/merge-log', newReport);
      if (res.data || res.success) {
        addToast('New snapshot report generated successfully!', 'success');
        fetchReportData();
      }
    } catch (err) {
      console.error(err);
      const mockLog = {
        id: Math.max(...reportLogs.map(l => l.id || 0), 0) + 1,
        action: 'System Performance Check',
        status: 'Success',
        details: 'Verified database connection pools, memory utilization, and active session counts.',
        created_at: new Date().toISOString()
      };
      setReportLogs(prev => [mockLog, ...prev]);
      addToast('New snapshot report generated (Local Mockup Mode)', 'success');
    }
  };
  ...
  const columns = [
    { 
      ...
      accessor: 'action',
      render: (row) => {
        const actionName = row.action || row.merge_reason || 'System Audit';
        return (
          <div className="flex items-center gap-3">
            <div className="p-2 bg-primary/10 text-primary rounded-lg">
              <FileText size={14} />
            </div>
            <span className="text-sm font-bold text-slate-800">{actionName}</span>
          </div>
        );
      }
    },
    ...
    { 
      ...
      render: (row) => {
        ...
        return <span className="text-xs font-bold text-slate-400">{new Date(dateVal).toLocaleDateString()}</span>;
      }
    },
    { 
      header: 'Status', 
  ...
  const filteredLogs = (reportLogs || []).filter(log => {
    const action = log.action || log.merge_reason || 'System Audit';
    return action.toLowerCase().includes(searchQuery.toLowerCase());
  });
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button icon={Plus} onClick={handleGenerateReport}>Generate Report</Button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {error ? (
          ...
          <Table 
            ...
            entriesPerPage={5}
            onEdit={(row) => handleDownloadReport(row)}
            renderActions={(row) => <Button variant="outline" size="sm" icon={Download} onClick={() => handleDownloadReport(row)}>PDF</Button>}
          />
        )}
      </div>
